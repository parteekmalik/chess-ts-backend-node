...
let useridwithsocket: { [key: number]: string } = {};
let socketidtouserandgameid: { [key: string]: { userid: number; gameid: number } } = {};

let games: { [key: number]: { players: { [key: string]: number }; game: Chess } } = { 12345: { players: { w: 1, b: 2 }, game: new Chess() } };
...
io.on("connection", (socket: any) => {
  ...
  socket.on("message", (msg: any) => {
    ...
    if (players[game.turn()] === userid) {
      try {
        game.move({ from, to });
        io.to(useridwithsocket[games[gameid].players["w"]]).emit("message-rcv", game.history()[game.history().length - 1]);
        io.to(useridwithsocket[games[gameid].players["b"]]).emit("message-rcv", game.history()[game.history().length - 1]);
      } catch {
        try {
  ...
  socket.on("connectwithuserid", (data: any) => {
    ...
    socketidtouserandgameid[socket.id] = { userid, gameid: 12345 }; 
    console.log(userid);
  }); 
